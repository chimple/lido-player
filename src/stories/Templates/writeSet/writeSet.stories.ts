...
function getContainerXml(args) {
  let tabCounter = 1;
  const { questions = [], isAllowOnlyCorrect = true, isContinueOnCorrect = true} = args;
  ...
  return `<main>
 <lido-container visible="true" show-next-button="true" dropAttr="math-matrix" appendToDropOnCompletion="true" equationCheck="$#mat1,==,$#number" objective="" is-allow-only-correct="${isAllowOnlyCorrect}" bg-color="transparent" bg-image="https://drive.google.com/file/d/1PTFrdNcsT01ioa9E-5qQGAPTwCc1lfnK/view" onInCorrect="lido-avatar.avatarAnimate='Fail'; this.sleep='2000';" onCorrect="lido-avatar.avatarAnimate='Success'; number.speak='true';  this.sleep='2000';" is-continue-on-correct="${isContinueOnCorrect}">
