...
| `isAllowOnlyOneDrop`     | `is-allow-only-one-drop`   | Determines whether only a single draggable element is allowed to be dropped. If set to 'true', only one draggable element can be dropped in the drop zone. If set to 'false', multiple draggable elements are allowed. | `string`  | `undefined` |
